...
def factorial(x):                                    # define a funtion called factorial
  factcalc = 1                                       # varialble to hold the result of the calculations as we pass through the loop - value set to 1 as this will not alter any multiplcation calculations
  counter = x                                        # using a local varialble for a counter otherwise the fuction will change the value of x - this may not be desirable if x is a global variable used elsewhere is a script 
...
testvalues = [5,7,10]                                # to test the funtion I created a list with the test values in it
